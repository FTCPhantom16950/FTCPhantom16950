...
public class ArtifactTrajectoryMath {
    ...
    public double[] calculateFly(double[] speedRange, double[] angelRange, double distance, double height, double mass, double g, double cd, double s, double p) {
        double[] res = new double[2];
        double dt = 0.01;
        List<double[]> minSpeedX = new ArrayList<>();
        int step0 = 0;
        for (double i = speedRange[0]; i <= speedRange[1]; i += 100) {
            int step1 = 0;
            for (double j = angelRange[0]; j <= angelRange[1]; j += 5) {
                double currentAngel = Math.toRadians(j);
                double currentSpeed = i, currentSpeedX = currentSpeed * Math.cos(currentAngel), currentSpeedY = currentSpeed * Math.sin(currentAngel);
                ...
                if ((travelledDistance >= distance) && (currentHeight <= height) && (travelledDistance <= distance + 0.35 )&& (currentHeight >= 0.75)) {
                    minSpeedX.add(new double[]{i,j,currentSpeedX});
                }
                step1 += 1;
            }
            step0 += 1;
